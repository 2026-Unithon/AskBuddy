...
**순서 원칙: 추출이 검색보다 먼저다.** 파이프라인은 둘이다 — 쓰기(자료 → 카드)와 읽기(질문 → 답변).
추출이 사실을 흘리면 카드에 그 내용이 없고, 검색은 없는 것을 찾지 못한다.
이때 지표는 "검색이 나쁘다"로 보이지만 오진이며, 임계값을 아무리 만져도 고쳐지지 않는다.

- 평가용 매장 자료를 먼저 확정한다. 승인 카드 30장 이상이 나와야 한다.
  카드가 3장이면 모든 지표가 1.0으로 나온다 — 잘해서가 아니라 너무 쉬워서다.
- **정답지를 두 벌 만든다.** 사실 정답지(원본 → 담긴 업무 사실)와 골든셋(질문 → 기대 카드).
  같은 읽기 작업에 얹어 한 번에 만든다.
- **사람 판정은 생략 불가다.** MRR·nDCG는 정답을 판별하지 않고 사람이 준 정답 대비 순서만 잰다.
  매장 사실은 점주 확인이 필수이고, 검색 관련성은 팀이 판정한다.
- **`E-O0` 추출 손실을 먼저 잰다.** 원본에 있는데 카드에 없는 비율이 시스템 정확도의 진짜 천장이다.
  여기서 30%를 흘리면 검색·생성을 완벽하게 해도 70%를 못 넘고, 검색을 고쳐서는 올라가지 않는다.
- 그다음 읽기 기준선(`E0`)을 찍고, 오라클 상한(`E-O1`~`E-O3`)으로 병목 단계를 찾는다.
- 전체 순환과 lifecycle을 자동화하고 CI 게이트를 건다.

완료 기준: 추출 손실과 읽기 기준선이 모두 저장되고, 실패 원인이 추출·검색·생성·검증·화면 중
어디인지 리포트에서 구분되며, 핵심 통합 시나리오가 CI에서 재실행된다.

## 14단계 — 품질 개선 (추출 → 카드 구조 → 검색 순서)

13단계의 `E-O0` 결과가 이 순서를 확정한다. 추출이 상류이므로 입력·추출 품질이 먼저다.

- 영상 원본 분석, STT timestamp, 2패스 추출, 원본 대조 검증과 근거 이미지를 구현한다.
- `RECIPE`와 `TASK`, 구조화 레시피, query variants, 출처 구분을 migration으로 도입한다.
- 앵커 소프트화, 용어사전 검색 연결, 접근 로그와 miss 분석을 구현한다.
- 공개 웹 자료에서 동의어·은어·오타·음성 인식 변형을 대량 수집하되 출처·신뢰도·검토 상태를 보존한다.
