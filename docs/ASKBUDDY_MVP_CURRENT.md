...
- 30~50문항 골든셋과 `/team/evaluations` 평가 하네스를 만든다.
- 정식 표현, 동의어, 은어, 오타, 음성 인식 변형, miss, 안전 질문, Q&A 관계 판정을 포함한다.
- 자료 → 카드 → 승인 → 로드맵 → 질문 → 점주 답변 → 재질문의 전체 순환을 자동화한다.
- 다중 사용자, 화면 복귀, 새로고침, 중복 클릭, 재시도, 네트워크 단절과 느린 응답을 검증한다.

완료 기준: 변경 전 기준 결과가 보존되고 핵심 통합 시나리오가 CI에서 재실행된다.
...
- 실제 영상·음성·PDF·이미지·텍스트를 자동 평가하고 Markdown/JSON 리포트를 생성한다.
- BM25/vector 후보 검색 뒤 LLM reranker를 필수 적용하되 기능 플래그, 비용·지연 기록, 안전 폴백을 둔다.
- 매장 격리, 조회 감사, rate limit, 레시피 유출 방지와 운영 관측성을 검증한다.

완료 기준: 기준선보다 품질이 개선되고 골든셋의 근거 없는 답변과 매장 교차 노출이 0이며, 실제 입력별 정확도·시간·비용 리포트가 남는다.
