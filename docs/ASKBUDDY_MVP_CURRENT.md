...
| 자료 우선순위 | 같은 사실이 여러 자료에서 다르게 나오면 **레시피북·공식 문서가 최우선**이다. 등급 순서는 레시피북·공식문서 > 점주 직접 답변 > 음성 인수인계 > 영상 > 카톡. 상위 등급 값을 기본 제안으로 올리되 **자동 확정하지 않는다.** 결정은 점주가 한다. |
| 자료 간 병합 | 같은 대상이 여러 자료에 걸쳐 들어오면 카드를 합친다. 판정은 `IDENTICAL/SUPPLEMENT/CONFLICT/NEW` 를 재사용하고, 보완·충돌은 기존 공개 카드를 자동으로 덮지 않고 점주 검수 제안으로 올린다. 같은 이름 다른 규격(HOT/ICE, 사이즈)은 **합치지 않는다.** |
...
- **자료 간 병합·충돌·점주 검수를 구현한다.** 같은 대상이 여러 자료에 걸쳐 들어오면 합치고,
  숫자가 부딪히면 점주 검수 제안으로 올린다. 결정 전에는 기존 공개본을 유지한다.
- 입력·추출 품질을 개선하고 `RECIPE`/`TASK` 카드 모델을 도입한다.

완료 기준: `E-O0`가 source type별로 측정되고, 여러 자료에 걸친 같은 대상이 한 카드로 합쳐지며,
숫자 충돌이 점주 검수로 올라가고 결정 전에는 기존 공개본이 유지된다.
