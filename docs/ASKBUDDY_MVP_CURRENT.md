...
| 카드 양식 | **고정 필드를 두지 않는다.** 칸을 정하면 안 맞는 사실은 버려지고 빈 칸은 지어내진다. 대신 **줄 문법**을 고정한다 — `제목`(대상+규격) / `수치줄`(숫자·수량) / `순서`(번호) / `목록`(나머지) / `근거`(자료+위치). 해당 사실이 없으면 그 줄이 통째로 빠진다. 줄 개수가 사실 개수를 따라가므로 담을 칸이 부족할 일이 없다. 자리가 고정돼 있어 알바가 어느 카드에서나 같은 위치에서 값을 찾는다. `RECIPE`/`TASK` 는 렌더러 힌트일 뿐 강제가 아니다 — 레시피는 수치줄이, 업무는 순서가 두꺼울 뿐 문법은 같다. |
| 카드 분량 | "한 화면"을 강제하지 않는다. 강제하면 다시 버리게 된다. 대신 **수치줄을 맨 위**에 둔다(질문 대부분이 여기서 답이 나온다). 목록이 8개를 넘으면 카드를 자르는 게 아니라 **대상을 쪼개라는 신호**로 본다. |
| 조립 검증 | 카드 조립도 서버가 검증한다. **카드의 모든 값이 원장의 사실에 있어야 한다.** D12(답변 생성은 카드에 없는 사실을 차단)와 대칭이다 — 답변은 카드 밖으로 못 나가고, 카드는 원장 밖으로 못 나간다. |
| 조립 손실 | 카드에 담기지 않은 사실(=`card_facts` 링크 없는 사실)을 **세고 드러낸다.** 숨기면 버그이고 드러내면 검수 항목이 된다. 점주 검수 화면에 "안 담긴 사실" 로 보여주고 `카드에 넣기`/`버리기` 를 고르게 한다. |
| 사실 원장 | **사실을 카드에서 떼어낸다.** 지금은 `facts.card_id` 가 카드에 종속돼(cascade) 카드를 다시 만들면 사실도 사라지고, 그래서 재조립이 곧 전면 교체가 된다. `source_facts` 를 자료 소유로 두고 append-only 로 보존하며, 카드는 그 사실들의 **현재 조립본(파생물)** 으로 본다. |
| 사실 검색 | **찾기는 사실로, 답하기는 카드로.** 카드 본문은 요약이라 숫자가 문장에 묻히지만 사실은 `대상/속성/값` 으로 분해돼 있어 "아이스라떼 우유 몇 ml" 에 정확히 맞는다(store-a 기준 사실 101건 vs 카드 70장). 다만 **사실로 답하지 않는다** — 점주는 카드를 검수했지 사실 하나하나를 검수하지 않았고, 불변식 3 은 카드 인용을 요구한다. 사실 → `card_facts` → 카드 → 승인·공개 여부 확인 → **카드 본문으로 답하고 카드를 인용**한다. |
| 사실 수정 | 카드에서 값이 바뀌면 **사실도 같이 바뀐다.** 다만 추출 기록과 현재 사실을 나눈다 — `value`(추출이 읽어낸 값)는 불변이고 `corrected_value`(점주가 고친 값)를 따로 둔다. 실제로 쓰이는 값은 `coalesce(corrected_value, value)` 이며 `effective_facts` 뷰가 그것이다. `value` 를 덮어쓰면 추출 품질(E-O0)을 잴 수 없다 — 프롬프트를 고쳐도 좋아졌는지 점주의 수정에 가려 보이지 않는다. 덤으로 "모델이 275 라 했는데 점주가 250 으로 고쳤다" 가 남아 프롬프트 개선의 재료가 된다. 수정 시각·수정자를 기록하고, 값이 아니라 사실 자체가 무효면 `is_superseded` 로 표시한다. |
| 증분 업로드 | 점주는 자료를 한 번에 올리기도 하고 그 뒤로 하나씩 계속 추가한다. 추가될 때마다 **새 사실의 대상(subject)을 다루는 카드만** 재조립하고 나머지는 건드리지 않는다. 재조립 결과는 `draft_version_id` 로만 올라가며 공개본은 점주 승인 시에만 바뀐다(D13). 점주가 승인·수정한 카드는 기존 본문을 기본 선택으로 둔다. |
| 추출 2패스 | map(자료별·구간별 원자 사실) → reduce(job 의 모든 사실을 모아 카드 조립). **reduce 는 자료 경계를 넘는다** — 영상의 제조순서와 레시피북의 수량이 한 카드로 모이려면 두 자료를 같이 봐야 한다. 프레임을 20→200 장으로 올려도 영상에서 새로 잡힌 사실이 0건이었다. 프레임이 아니라 한 호출에 담는 양이 한계다. |
...
| 추출 온도 | **0.0 으로 고정한다.** 답변 생성과 같다. 0.2/0.4/0.6 을 각 3회 실측했으나 이득이 없고 판정 불안정만 늘었다. 추출은 원문의 사실을 옮기는 일이라 다양성이 필요 없고, 온도는 원문에 없는 표현을 만들 여지만 준다. |
| 실험 판정 | 추출이 비결정적이라(온도 0.0 에도 손실률 18.8%p 변동) 집계값 비교는 성립하지 않는다. **사실 단위 짝비교 · 최소 3회 · 불안정 사실 판정 제외 · 음성 대조군 필수.** 대조군 오차보다 작은 차이로 설정을 바꾸지 않는다. |
...
- **원장이 먼저고 카드가 나중이다.** map 이 뽑은 사실을 원장에 먼저 쓰고, reduce 가 그
  사실을 읽어 카드를 조립한 뒤 담은 것만 `card_facts` 로 잇는다. 링크가 없는 사실이
  곧 조립 손실이며, 그것이 검수 화면의 "안 담긴 사실" 이 된다.
