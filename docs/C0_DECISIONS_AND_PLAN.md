...
> W/R 병합 후 답변 검증 접점 통합: [C0_WR_ANSWER_BOUNDARY.md](review/C0_WR_ANSWER_BOUNDARY.md). 공통 참조/R 적합성 역할·합성 연결 인수·병렬 착수 범위를 기록한다. 아래 CP-00 선행과 전체 인수 기준은 유지한다.
...
> R 구현 진행: [2026-09-14 구현 기록](review/C0_R_IMPLEMENTATION_20260914.md). CP-01 R typed 계약/참조 검증·인증 및 기존 읽기 계측 결함을 보강했다. 공통 usage/RAW·DB 선행은 미완료이며 C0 전체 완료가 아니다.
...
`api/app/contracts/`와 기존 계약 테스트 22개는 재사용한다. 공개·답변 런타임은 아직 이 모듈을 소비하지 않는다. 새 타입 존재와 C0 전체 완료를 구분한다. [검토 결과](review/C0_REVIEW_20260914.md)에 재현 근거를 남긴다.

원가 세부 계약·코드 접점·검증은 [C0_COST_MEASUREMENT_PLAN.md](plan/C0_COST_MEASUREMENT_PLAN.md), 입력 시나리오는 [c0_cost_scenarios.json](c0_cost_scenarios.json)에 고정한다. U1 비용 범위는 해결됐고 U2는 미래 사용량 회신 대기 대신 시나리오 + 실제 메타데이터 계측으로 전환했다.
