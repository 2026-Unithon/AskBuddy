# AskBuddy 통합 설계 최종본 v5

> 개정일: 2026-09-12
> 구현 기준 커밋: `d62c08e` (`main`)
> 운영 구성: Supabase + Railway API + Vercel Web
> 문서 성격: 제품 결정, 데이터·API 계약, 화면 흐름, 구현 현황, 검증 및 배포 기준을 하나로 합친 최신 기준서

이 문서는 다음 자료를 현재 대화에서 확정된 결정에 맞게 재구성한 것이다.

- `AskBuddy 설계최종본 v4.md`
- `ASKBUDDY_MVP_CURRENT.md`
- 저장소 `docs/ASKBUDDY_MVP_CONTRACT_V1.md`
- 저장소 `docs/DEPLOYMENT_RUNBOOK.md`
...
1. 가장 최근 사용자 결정
2. 이 문서의 확정 설계
3. `ASKBUDDY_MVP_CONTRACT_V1.md`의 데이터·API 계약
4. migration과 현재 동작 코드
5. 이전 v4 및 과거 회의 자료
...
| 유사 카드 | `IDENTICAL`, `SUPPLEMENT`, `CONFLICT`, `NEW`로 판정한다. 동일은 연결, 보완·충돌은 검토 제안, 신규는 안전할 때 공개한다. |
...
기준 커밋은 `d62c08e`이며 운영 migration은 `20260911130000_notification_delivery_contract.sql`까지 적용·검증된 상태다.
...
## 11단계 — 프론트 서버 상태·Lifecycle 안정화

상태: 구현 완료. 실제 인증 계정·느린 작업·화면 복귀 E2E는 13단계에서 반복 검증한다.

- TanStack Query 도입과 owner/staff layout provider 구성
- query key 표준화: 사용자·매장·자원 ID 포함
- 업로드 작업을 접수와 완료 대기로 분리
- `/ingest/jobs` 기반 재진입·새로고침 복원
- 완료 시 cards/jobs/notifications 자동 invalidation
- raw effect fetch, 화면 setInterval 폴링, mock 성공 제거
- Context/localStorage에서 서버 데이터 분리
- typed ESLint, React hooks 규칙, Promise 규칙, `--max-warnings 0`

완료 기준: 다른 화면으로 갔다 돌아오거나 새로고침해도 처리 상태가 풀리지 않고, 완료 결과가 새로고침 없이 반영된다.

## 12단계 — 신규 계약의 Web 화면 완전 연결

상태: 구현 완료. 실기기·다중 사용자·실데이터 회귀는 13단계 범위다.

- 사장님 3탭 정보 구조 정리
- 신규 카드 목록·상세·초안·승인·제외·복원·수동 이동 연결
- 작업별 카드 검토와 오래된 딥링크 처리
- 지식 제안 `SUPPLEMENT/CONFLICT` 검토 화면
- 직원 FAQ 버튼·목록·근거 답변 연결
- 알림 배지·목적지·읽음·업무 완료 상태 연결
- 모든 목록의 loading/empty/error/retry/background-refresh UI

완료 기준: 신규 API가 구현돼 있어도 화면에서 접근할 수 없는 기능이 남지 않는다.

## 13단계 — 통합·E2E·실기기 검증

- 20-1 Q&A·분류 회귀 세트
- 20-2 비동기 Lifecycle E2E
- 사장님·여러 직원의 동일 질문 및 답변 전파
- 중복 클릭·재시도·네트워크 단절·느린 응답
- iOS 홈 화면 Web Push, Android/Chrome, 데스크톱 브라우저
- 권한 거절·구독 만료·Push 실패에서도 앱 내부 알림 유지
- 모바일 360×800, 390×844와 긴 카드·30개 이상 목록

완료 기준: 정상 경로뿐 아니라 실패·복귀·중복·권한·긴 작업이 자동 테스트와 실기기 체크에서 통과한다.

## 14단계 — 품질·보안·운영 평가

- 최소 30문항 골든셋과 실제 업로드 자료 정답 기준
- 생성 답변 citation·숫자·부정 표현 검증
- 매장 간 권한 격리와 레거시 `/reg/*` 정리
- rate limit, 파일 크기·길이·페이지 제한 실측
- 추출 시간, hit 응답 시간, 변동 비용 측정
- 로그의 request/job/prompt/model/version 추적
- 운영 데이터에서 읽기 전용 무결성 검증

완료 기준: 오답·정보 누출·매장 교차 접근이 0이며 성능·비용 수치를 실제 측정값으로 기록한다.

## 15단계 — 운영 인수·최종 승인

- 운영 DB 백업 후 최종 migration 적용과 verify
- Railway/Vercel 자동 배포 및 health/CORS/핵심 플로우 smoke test
- 기존 사용자 데이터 보존 확인
- 데모 계정으로 자료 → 카드 → 로드맵 → 질문 → 점주 답변 → 재질문 전체 관통
- 배포 런북·장애 대응·롤백·환경변수 목록 최신화
- 문서의 구현 현황을 최종 결과와 맞춤
- Figma가 있으면 기능 계약을 유지한 채 시각 QA

완료 기준: 운영 URL에서 새로고침 없이 전체 순환이 동작하고, 알려진 차이가 문서화되며, 15단계 이후 DB 운영 권한 정책으로 전환된다.
