...
R v2 명시 확인 접점(2026-09-16): 현재 구현 경로 `POST /learn/v2/chat`의 선택 필드 `policy_receipt_id`는 같은 매장·회원·세션에 저장된 SAFE_ROUTE 응답만 참조한다. `question`은 해당 응답의 원래 질문과 정확히 같아야 하며 명확화 선택과 함께 보낼 수 없다. 참조 없는 정책 안내는 기존대로 pending을 만들지 않는다. 명시 요청은 추가 모델 호출 없이 ESCALATE로 원문·정책 응답 연결·pending·알림을 원자 저장한다. 동일 요청 키는 재생하고, 다른 키로 같은 정책 응답을 중복 이관하면 409다. REFUSE는 이 접점으로 우회하지 않는다. 직원 화면은 최신 안전 안내에 확인 버튼을 제공하며, 확인 요청 자체를 안전 판정으로 표시하지 않는다.

직원 앱 알림은 `/staff/notifications/v2`에서 조회·읽음 처리하고 해당 v2 대화로 이동한다. 서버 수신자/매장 권한 검사가 정본이며, 화면은 서버가 발행하는 직원 v2 대화 경로만 연다. 앱 읽음과 Web Push 요청/수신은 별개다. 이 화면 연결은 Push 재전달 worker 운영 연결의 완료를 뜻하지 않는다.
