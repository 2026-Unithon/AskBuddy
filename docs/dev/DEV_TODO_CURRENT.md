...
2026-09-19 일반 의미 런타임: [미등록 자유 표현 제안→서버 검사→별도 모델 검증→기존 저장](review/R_GENERAL_RUNTIME_20260919.md)을 기본 OFF로 구현했다. 질문별 catalog 없이 동작하는 경로이며 실제 모델·코드·승인판 품질 인수 release가 필요하다. 구현 준비와 의미 정확성·운영 활성화는 구분한다.

2026-09-19 R 단독 후속: [통합 평가 예산·사람 판정 catalog 연결](review/R_SOLO_FOLLOWUP_20260919.md). QUERY/EMBED/RERANK/ANSWER 공통 예산 예약과 v2 오류 분리, 검토 catalog 생성·재검증 도구를 구현했다. 아래 통합 예산 미구현 문구는 이전 시점 기록이다. 실제 사람 검토/유료 실행/일반화 품질·W 공동 인수는 대기다.
