...
async def main():
    ...
    # CP-01 이후 승인 묶음은 불변이라 반례를 새로 지어 만든다 (RV-07)
    unrelated = {**snapshot().fact_revisions[0].model_dump(),
                 "fact_revision_id": "7", "predicate": "other",
                 "original_assertion": "별도 업무 사실", "assertion": "별도 업무 사실"}
    snap = rebuilt(extra_facts=[unrelated],
                   extra_blocks=[dict(block_id="unrelated", kind="QUANTITIES",
    ...
    snap = rebuilt(fact_changes={"conditions": ["포장 주문에만 적용"],
                                 "exceptions": ["특별 행사일 제외"]})
    try:
