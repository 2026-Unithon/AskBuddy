...
class _RecoveryStore:
    """`ingest_job_sources.recovery_state` 한 행을 흉내 낸다.

    같은 작업의 두 실행(run_tag 만 다름)은 이 CAS 상태를 공유해야 두 번째
    호출이 첫 실행이 남긴 `failed_segment_ids` 를 보고 재시도 구간을 잡는다 —
    process_source 는 명시로 준 retry_segments 를 신뢰하지 않고
    COMMITTED phase 의 recovery_state 로 덮어쓴다.
    """

    def __init__(self):
        self.state = None

    async def load(self, conn, store_id, job_id, source_id, *, lock=False):
        return self.state

    async def replace(self, conn, store_id, job_id, source_id, expected, state):
        if self.state != expected:
            raise pipeline.recovery.RecoveryConflict('다른 처리에서 복구 상태를 변경했습니다.')
        self.state = state


async def _run_with_ledger(tmp_path, ledger, *, run_tag, retry_segments=None,
                           fail_segments=(), store=None):
    """구간 3개 영상 자료를 한 번 처리한다. 추출·조립이 receipt 를 먼저 적는다."""
    extracted, assembled = [], []
    store = store if store is not None else _RecoveryStore()
    ...
    with patch.object(pipeline, "get_pool", return_value=_Pool()), \
         patch.object(pipeline.recovery, 'load', store.load), \
         patch.object(pipeline.recovery, 'replace', store.replace), \
         patch.object(pipeline, "_preprocess",
                      AsyncMock(return_value=("본문", [], SEGMENTS))), \
         patch.object(pipeline.repo, "get_source", AsyncMock(return_value=src)), \
         patch.object(pipeline.repo, "set_status", set_status), \
...
async def test_same_job_retry_with_new_run_tag_does_not_collide(tmp_path):
    ...
    # 잃은 구간은 명시 인자가 아니라 첫 실행이 남긴 recovery_state(COMMITTED)
    # 로 정해지므로 store 를 그대로 이어 쓴다
