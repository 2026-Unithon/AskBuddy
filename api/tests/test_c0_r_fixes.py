...
from tests.test_c0_read_contract import QUERY, answer, rebuilt, snapshot
...
class SelectionFixTest(unittest.TestCase):
    def test_unrelated_block_rejected_but_required_block_allowed(self):
        # CP-01 이후 승인 묶음은 불변이라 반례를 변형이 아니라 새로 지어 만든다 (RV-07)
        extra = {**snapshot().fact_revisions[0].model_dump(),
                 "fact_revision_id": "7", "predicate": "preparation",
                 "original_assertion": "필수 준비", "assertion": "필수 준비"}
        block = dict(block_id="preparation", kind="QUANTITIES", order=2,
                     fact_revision_ids=["7"])
        snap = rebuilt(extra_facts=[extra], extra_blocks=[block])
        ...
        # 선행 사실로 묶이면 같은 계획이 통과한다
        linked = rebuilt(extra_facts=[extra], extra_blocks=[block],
                         fact_changes={"requires": ["7"]})
        validate_answer_plan(plan, linked, QUERY, store_id=1)

    def test_conditions_and_exceptions_fail_closed_until_supported(self):
        for field in ("conditions", "exceptions"):
            snap = rebuilt(fact_changes={field: ["포장 주문"]})
