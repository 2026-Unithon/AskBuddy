      ...
      <div className="flex-1 space-y-4 overflow-y-auto px-5 pb-8">
        ...
        {data && <>
          <Card className="space-y-4 p-5">
            ...
            <div className="grid grid-cols-4 gap-2 text-center">{[["자료", data.counts.sources], ["성공", data.counts.succeeded], ["실패", data.counts.failed], ["카드", data.counts.cards]].map(([label, value]) => <div key={label} className="rounded-xl bg-surface-muted p-2"><p className="text-base font-bold">{value}</p><p className="text-xs text-muted">{label}</p></div>)}</div>
            {isIngestJobActive(data.status) && <p className="text-xs text-muted">다른 화면으로 이동해도 서버에서 계속 처리합니다.</p>}
            {(data.status === "FAILED" || data.status === "NO_RESULT") && <Button loading={retry.isPending} loadingLabel="작업 재시도 중" onClick={() => retry.mutate()}>작업 재시도</Button>}
