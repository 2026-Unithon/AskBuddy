...
> pull `65c2403` 이후 C0 결정·담당·PR 선행 관계는 `docs/C0_DECISIONS_AND_PLAN.md`, 재현 검토는 `docs/C0_REVIEW_20260914.md`를 읽는다. 타입 기반·기존 22개 unit은 확인됐으나 C0 전체 완료는 아니다.
> 사용자 원가 회신 반영: `docs/C0_COST_MEASUREMENT_PLAN.md`의 CP-00A~C 원가 계측을 먼저 구현한다. 미래 사용량은 첫 달/안정기 시나리오로 다루며 실제 metadata·비용을 수집한다.
...
| D18 | **승격 기준.** 순증 ≥ 대조군 폭 × 1.5 (최소 5건) AND `must_have` 악화 0건 AND 원장 재현율 하락 없음 AND 3회 중 2회 이상 같은 방향. 원가 중립은 대조군 폭 문턱(최소 5건 유지). 대표 순증 median·A/A 폭·전체 고정 분모는 실험설계 §9-1. 통계적 유의성 증명이 아니며 과거 잡음 수치를 새 기준선으로 사용하지 않는다 |
| D19 | **같은 메뉴의 다른 규격은 카드 하나에 담되 값은 나란히 둔다.** `HOT 275ml · ICE 225ml` 처럼 규격별 값을 보존하고 범위로 뭉개지 않는다. 답할 때는 `CLARIFY` 로 규격을 확정한 뒤 그 값만 인용한다. 사실(`source_facts`)은 여전히 `variant` 로 갈라 저장한다 — 카드는 표시 단위이고 사실이 값의 단위다 |
...
2026-09-14 기존 런타임 대조 `0b8e1c4`, 추가 C0 타입 기반·D18~D21 대조 `65c2403`. 카드 수명주기·동적 로드맵·질문/점주 답변·알림·team 평가·사실 원장 migration 기반이 존재한다. 파일 존재·과거 테스트 기록·운영 검증을 구분한다.

CP-00A~C·CP-01~03과 CP-04의 W 몫(M0·M1 migration, 발행 트랜잭션 서비스)이 구현·검증됐다 — 282 unit, C0 probe 19/19 GAP 0, 실제 DB 시나리오 12/12, 빈 DB 재구축 성공. 남은 것은 R 몫(CP-00B의 embed/answer 계측, M2 색인 staging, M3 v2 session/context)과 CP-05, 그리고 W0~W5 / R0~R5 → J0~J3이다.

**구현이 통과는 아니다.** 원가 D21 판정은 요율표·답변 단가·저장비·환율이 없어 UNDETERMINED이며 통과로 읽지 않는다. 제품 runtime이 새 계약을 소비하는 경로는 아직 연결되지 않았다 — 원장 선저장·불변 승인 snapshot·참조 renderer·chat v2는 계약과 fake 수준이지 런타임 구현이 아니다. 상세 현황은 MVP 22-5절, 완료 조건은 TODO를 따른다.
