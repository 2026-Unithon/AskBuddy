...
| D18 | **승격 기준.** 순증 ≥ 대조군 폭 × 1.5 (최소 5건) AND `must_have` 악화 0건 AND 원장 재현율 하락 없음 AND 3회 중 2회 이상 같은 방향. 원가가 늘지 않는 변경은 문턱을 대조군 폭까지 낮춘다. 실측 잡음이 사실 수의 8~10%였고(store-a ±11/132, store-b ±5/48) 이미 두 번 속았다 — 프레임 실험에서 대조군 −11 대 실험 −2, 60장 대 64장이 +5 |
...
| D20 | **자료를 지워도 사실과 카드는 남는다.** 이미 나간 답변의 인용은 끊지 않고 `인용 끊김` 으로 표시한다. append-only 표현만으로 삭제 정책을 덮지 않는다 (개인정보 삭제 요청은 별도 절차) |
| D21 | **원가·지연 상한.** 매장당 월 3,000원(구독 9,900원의 30%), 답변 `p95` 5초. 카드 생성 지연은 보류한다 — 백그라운드 작업이라 사용자를 막지 않는다 |
