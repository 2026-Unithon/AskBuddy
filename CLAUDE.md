...
| D18 | **승격 기준 — 역할 분리.** 성능은 반복별 순증 Δᵢ 의 **중앙값**, 안정성은 매번 성공/매번 실패/변동을 별도 보고, 안전성은 **모든 반복**의 악화를 확인하며 불안정 사례도 빼지 않는다. `안정(성공)→갈림` 은 그 자체가 차단 조건이다 — 만장일치만 쓰면 `성공/성공/성공 → 성공/성공/실패` 가 '악화 0건' 뒤에 가려진다. 미판정 필수 사례가 있으면 '악화 0건 확인' 으로 적지 않는다. 최소 3회·2/3 방향 일치·최소 개선량은 개발 단계 판단 규칙이며 **통계적 확신이 아니다.** 반복을 늘려도 최댓값−최솟값은 줄지 않는다. 실험 전에 정답·채점기·snapshot 버전·분모·반복 수·실패 처리·대표 개선량을 고정한다 (실험설계 §9-3) |
...
2026-09-15. C0 는 CP-00A~C·CP-01~04(W 몫)와 R 답변 검증 접점까지 구현·검증됐다.
W0 정답지 감사와 기준선 측정이 끝났고(**분모 278건**), W1 에서 파이프라인을
`입력 → 사실 → 원장 → 조립 → 카드` 로 바꿨다.
...
상세 현황은 MVP 22-5·22-6절, 완료 조건은 TODO 를 따른다.
