...
| D16 | 추출 온도 `EXTRACT_TEMPERATURE` **0.0**. 답변 생성(D12)과 같다. 0.2/0.4/0.6 을 각 3회 실측한 결과 이득이 없고(순증 +1/−1/−2) 판정 불안정만 늘었다. 추출은 원문의 사실을 옮기는 일이라 다양성이 필요 없다 |
| D17 | 추출은 비결정적이다. 온도 0.0·전사문 고정에도 손실률이 **18.8%p** 흔들린다. 그래서 품질 실험은 ① 집계값이 아니라 **사실 단위 짝비교**(`compare_runs.py`) ② 실험군마다 **최소 3회** ③ 다수결이 갈리는 사실은 판정 제외 ④ **음성 대조군 필수**. 대조군 오차보다 작은 차이로 설정을 바꾸지 않는다 |
